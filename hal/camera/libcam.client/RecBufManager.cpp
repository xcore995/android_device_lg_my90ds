...
*
*******************************************************************************/
#define MY_LOGV(fmt, arg...)        ALOGV("(%s)[RecBufManager::%s] " fmt, getName(), __FUNCTION__, ##arg)
#define MY_LOGD(fmt, arg...)        ALOGD("(%s)[RecBufManager::%s] " fmt, getName(), __FUNCTION__, ##arg)
#define MY_LOGI(fmt, arg...)        ALOGI("(%s)[RecBufManager::%s] " fmt, getName(), __FUNCTION__, ##arg)
#define MY_LOGW(fmt, arg...)        ALOGW("(%s)[RecBufManager::%s] " fmt, getName(), __FUNCTION__, ##arg)
#define MY_LOGE(fmt, arg...)        ALOGE("(%s)[RecBufManager::%s] " fmt, getName(), __FUNCTION__, ##arg)
#define MY_LOGA(fmt, arg...)        ALOGA("(%s)[RecBufManager::%s] " fmt, getName(), __FUNCTION__, ##arg)
#define MY_LOGF(fmt, arg...)        ALOGF("(%s)[RecBufManager::%s] " fmt, getName(), __FUNCTION__, ##arg)
//
...
)
{
    ...
    if(rpImgInfo->mbMetaMode)
    {
        ...
        ALOGD("[RecImgBuf::alloc][GraphicBuffer] Size(%d x %d), F(0x%X), BU(0x%X)",
    ...
    #if REC_BUF_ION
    //
    ALOGD("[RecImgBuf::alloc][mt_ion_open]");
    IonDevFd = mt_ion_open(LOG_TAG);
    if(IonDevFd < 0)
    {
        ALOGE("[RecImgBuf::alloc]ion_open fail");
        goto lbExit;
    }
    //
    ALOGD("[RecImgBuf::alloc][ion_alloc_mm]IonDevFd(%d), ImgBufSize(%d)",
            IonDevFd,
            ImgBufSize);
    if(ion_alloc_mm(
        IonDevFd,
        ...
        &IonHandle))
    {
        ALOGE("[RecImgBuf::alloc]ion_alloc_mm fail");
        goto lbExit;
    }
    //
    ALOGD("[RecImgBuf::alloc][ion_share]");
    if(ion_share(
        IonDevFd,
        IonHandle,
        &IonBufFd))
    {
        ALOGE("[RecImgBuf::alloc]ion_share fail");
        goto lbExit;
    }
    //
    ALOGD("[RecImgBuf::alloc][requestMemory] fd(%d), S(%d)", IonBufFd, ImgBufSize);
    camera_memory = requestMemory(IonBufFd, ImgBufSize, 1, NULL);
    if  ( ! camera_memory )
    {
        ret = false;
        ALOGE("[RecImgBuf::alloc][requestMemory] id:%d, size:%d", IonBufFd, ImgBufSize);
        goto lbExit;
    }
    //
    #else
    //
    ALOGD("[RecImgBuf::alloc][MemoryHeapBase]");
    sp<MemoryHeapBase> pMemHeapBase = new MemoryHeapBase(ImgBufSize, 0, rpImgInfo->ms8ImgName);
    if  ( pMemHeapBase == 0 )
    {
        ALOGE("[RecImgBuf::alloc][PrvCbImgBuf::alloc] cannot new MemoryHeapBase");
        goto lbExit;
    }
    //
    ALOGD("[RecImgBuf::alloc][requestMemory]");
    camera_memory = requestMemory(pMemHeapBase->getHeapID(), ImgBufSize, 1, NULL);
    if  ( ! camera_memory )
    {
        ret = false;
        ALOGE("[RecImgBuf::alloc][requestMemory] id:%d, size:%d", pMemHeapBase->getHeapID(), ImgBufSize);
        goto lbExit;
    }
    //
    pMemHeapBase = 0;
...
{
    #if REC_BUF_ION
    ALOGD("[RecImgBuf::RecImgBuf]ION");
    #else
    ALOGD("[RecImgBuf::RecImgBuf]M4U");
    #endif

    if(mpImgInfo->mbMetaMode)
    {
        ALOGD("[RecImgBuf::RecImgBuf]gralloc on");
        //
        ...
        if ( mi4BufIonFd < 0 )
        {
            ALOGD("[RecImgBuf::RecImgBuf] fd:%d <= 0", mi4BufIonFd);
        }

        if(mpImgInfo->mu4ImgFormat == HAL_PIXEL_FORMAT_YV12)
        {
            mpImgInfo->ms8ImgFormat = "yuv420p";
        }
        else
        if(mpImgInfo->mu4ImgFormat == HAL_PIXEL_FORMAT_I420)
        {
            mpImgInfo->ms8ImgFormat = "yuv420i-yyuvyy-3plane";
        }
        else
        {
            mpImgInfo->ms8ImgFormat = "yuv420p";
        ...
        ALOGD("[RecImgBuf::RecImgBuf][handle] mpGraphicBuffer->handle: 0x%8x", mpGraphicBuffer->handle);
        ALOGD("[RecImgBuf::RecImgBuf][callbackMem] fd(%d), mem(0x%X), VA(0x%X), S(%d)",
                mIonBufFd,
                (uint32_t*)&mCamMem,
                (uint32_t*)mCamMem.data,
                mCamMem.size);
        ALOGD("[GrallocMem] W(%d), H(%d), F(%s), fd(%d), stride{%d, %d, %d}, S(%d)",
                getImgWidth(),
                getImgHeight(),
                getImgFormat().string(),
    ...
    {
        ALOGD("[RecImgBuf::RecImgBuf]gralloc off");
    }
